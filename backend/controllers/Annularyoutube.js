import annularModel from '../models/AnnularcutterModel.js'

export const createdannular = async (req, res) => {
  try {
    ...
    // Duplicate Url check
    const urlExist = await annularModel.findOne({ youtubeurl })
    if (urlExist) {
    ...
    // Save blog
    const uploaddewalt = new annularModel({
      title,
      youtubeurl,
      status,
    })
...
// ======================= GET ALL Youtube =======================
export const getallannular = async (req, res) => {
  try {
    const dewalt = await annularModel.find().sort({ createdAt: -1 }) // latest first
    res.status(200).json({
      data: dewalt,
      success: true,
      error: false,
...
export const deleteannular = async (req, res) => {
  try {
    const { id } = req.params

    const deleteyoutube = await annularModel.findByIdAndDelete(id)
...
// ======================= UPDATE Category =======================
export const updateyoutubeannular = async (req, res) => {
  try {
    const { id } = req.params
...
// Update blog status
export const updateStatusannular = async (req, res) => {
  try {
    const { id } = req.params
    const { status } = req.body

    const video = await annularModel.findById(id)
    if (!video) {
