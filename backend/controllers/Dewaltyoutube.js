import dewaltmodel from '../models/dewalthistoryModel.js'

export const createdewalt = async (req, res) => {
  try {
    ...
    // Duplicate Url check
    const urlExist = await dewaltmodel.findOne({ youtubeurl })
    if (urlExist) {
    ...
    // Save blog
    const uploaddewalt = new dewaltmodel({
      title,
      youtubeurl,
      status,
    })
...
// ======================= GET ALL Youtube =======================
export const getalldewalt = async (req, res) => {
  try {
    const dewalt = await dewaltmodel.find().sort({ createdAt: -1 }) // latest first
    res.status(200).json({
      data: dewalt,
      success: true,
      error: false,
...
export const deletedewalt = async (req, res) => {
  try {
    const { id } = req.params

    const deleteyoutube = await dewaltmodel.findByIdAndDelete(id)
...
// Update blog status
export const updateYoutubeStatus = async (req, res) => {
  try {
    const { id } = req.params
    const { status } = req.body

    const video = await dewaltmodel.findById(id)
    if (!video) {
