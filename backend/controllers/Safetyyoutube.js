import safetyModel from '../models/safetyyoutubeModel.js'

export const createdsafety = async (req, res) => {
  try {
    ...
    // Duplicate Url check
    const urlExist = await safetyModel.findOne({ youtubeurl })
    if (urlExist) {
    ...
    // Save blog
    const uploaddewalt = new safetyModel({
      title,
      youtubeurl,
      status,
    })
...
export const getallsafety = async (req, res) => {
  try {
    const dewalt = await safetyModel.find().sort({ createdAt: -1 }) // latest first
    res.status(200).json({
      data: dewalt,
      success: true,
      error: false,
...
export const deletesafety = async (req, res) => {
  try {
    ...
    const deleteyoutube = await safetyModel.findByIdAndDelete(id)
...
// ======================= UPDATE Category =======================
export const updateyoutubesafety = async (req, res) => {
  try {
    const { id } = req.params
...
export const updateStatus = async (req, res) => {
  try {
    ...
    const video = await safetyModel.findById(id)
    if (!video) {
